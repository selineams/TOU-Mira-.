...
public static class GameManagerPatches
{
    ...
    public static bool DidHumansOrImpostorsWinPatch(GameManager __instance, GameOverReason reason, ref bool __result)
    {
        ...
        {
            winType = 1;
            GameHistory.WinningFaction = $"<color=#{Palette.CrewmateBlue.ToHtmlStringRGBA()}>船员</color>";
        }
        ...
        {
            winType = 2;
            GameHistory.WinningFaction = $"<color=#{Palette.ImpostorRed.ToHtmlStringRGBA()}>内鬼</color>";
        }
